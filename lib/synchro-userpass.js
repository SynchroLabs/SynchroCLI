...
wait.launchFiber(function()
{
	...
	if (!userName)
	{
		userName = wait.for(read, {prompt: "User name: "});
	}
	...
	if (!command.delete)
	{
		...
		if (!userPass)
		{
			userPass = wait.for(read, {prompt: "Password: ", silent: true, replace: "*"});
			var userPass2 = wait.for(read, {prompt: "Verify password: ", silent: true, replace: "*"});
