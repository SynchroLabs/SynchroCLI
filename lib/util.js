...
exports.getAppsConfig = function * (config, moduleStore)
{
    ...
    {
        console.log("Save, this.moduleFile:", this.moduleFile);
        if (this.moduleFile)
        {
...
function getDownloadUrl(command)
{
	// host - Priority: command line parameter, npm config, default (blob.synchro.io/dist)
	//
	var location = "http://blob.synchro.io/dist";
	if (command.downloadServer)
	{
		location = command.downloadServer;
	}
	else
	{
		var locationFromNpm = npm.config.get("synchro-download-server");
		if (locationFromNpm)
		{
			location = locationFromNpm;
		}
	}
...
exports.downloadAndInstallServer = function * (command)
{
    ...
    var downloadPath = getDownloadUrl(command) + serverPath; 

	console.log("Downloading server from: " + downloadPath);

    // Tell Google Analytics that we're downloading...
    //
    var visitor = ua('UA-62082932-1');
    visitor.pageview('/dist' + serverPath).send();

	var fileContents = yield utilWaitFor(exports.getUrl, downloadPath);
