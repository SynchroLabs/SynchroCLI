...
co(function * ()
{
	...
	if (/^https?:.*/.test(appReference))
	{
		...
		fileContents = yield util.waitFor(util.getUrl, appReference);
	}
	else
	{
			...
			if (packageJson)
			{
				...
				if (packageJson.engines && packageJson.engines.synchro)
				{
					if (appContainer)
					{
						...
						else if (yield util.containerExists(modulesStore, appName))
						{
					...
					else
					{
						...
						//
						console.log("Synchro app '%s' dependency installation via npm install...", appName);
						yield util.installAppDependencies(config, appName);
					}
				}
				else
				{
