...
command.usage('[options] <uri> <appname>');
...
command.on('--help', function()
{
    console.log('  Details:');
    console.log('');
    console.log('    The uri provided should point to a file that was created by running "npm pack" on a Synchro app.');
    console.log('');
...
//
// !!! TODO: It would be nice to be able to install from a variety of sources, like npm install (folder, tarball file, etc).
//     It might also be nice to support some kind of short name reference to an app hosted in a synchro.io repository of 
//     apps (sample and/or demo apps).
//
...
wait.launchFiber(function()
{
	...
	if (isGzip(file))
	{
		var unzipped = wait.for(zlib.unzip, file);
		if (isTar(unzipped))
		{
