...
co(function * ()
{
	...
	function * dumpApp(appName, appContent, showDetails)
	{
		// App content: { "container": "foo" }
		//
		if (appName != appContent.container)
		{
			console.log("Application: %s [container: %s]", appName, appContent.container);
		}
		else
		{
	...
	{
		var appName = util.installedAppNamesFromContainer(config, container);
		...
		{
			console.log("Container: %s [installed as %s]", container, appName);
		}
		else
		{
	...
	else
	{
	    ...
	    if (Object.keys(apps).length > 0)
	    {
	    	for (var app in apps) 
	    	{
				if (!appName || app.uriPath.match(appName))
				{
					if (command.details)
					{
						// Separator (between apps) for detailed output
